...
export const metadata: Metadata = { title: 'Receptionist followups — Pixie Lab', robots: { index: false, follow: false } };
export const dynamic = 'force-dynamic';

export default async function Page() {
  const guard = await guardPermission('receptionist.view');
  if (!guard.ok) return <AccessRestricted what="Receptionist" />;
  return <ReceptionistWorkspace tab="followups" tenant={tenantForMembership(guard.membership)} />;
}
