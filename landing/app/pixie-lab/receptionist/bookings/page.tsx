...
export const metadata: Metadata = { title: 'Receptionist bookings — Pixie Lab', robots: { index: false, follow: false } };
export const dynamic = 'force-dynamic';

export default async function Page() {
  const guard = await guardPermission('receptionist.view');
  if (!guard.ok) return <AccessRestricted what="Receptionist" />;
  return <ReceptionistWorkspace tab="bookings" tenant={tenantForMembership(guard.membership)} />;
}
