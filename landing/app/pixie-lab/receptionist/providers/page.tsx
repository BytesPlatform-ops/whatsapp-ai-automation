...
export const metadata: Metadata = { title: 'Receptionist providers — Pixie Lab', robots: { index: false, follow: false } };
export const dynamic = 'force-dynamic';

export default async function Page() {
  const guard = await guardPermission('receptionist.view');
  if (!guard.ok) return <AccessRestricted what="Receptionist" />;
  return <ReceptionistWorkspace tab="providers" tenant={tenantForMembership(guard.membership)} />;
}
