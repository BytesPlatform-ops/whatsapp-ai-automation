...
export const metadata: Metadata = { title: 'Receptionist analytics — Pixie Lab', robots: { index: false, follow: false } };
export const dynamic = 'force-dynamic';

export default async function Page() {
  const guard = await guardPermission('receptionist.view');
  if (!guard.ok) return <AccessRestricted what="Receptionist" />;
  return <ReceptionistWorkspace tab="analytics" tenant={tenantForMembership(guard.membership)} />;
}
