...
export const metadata: Metadata = { title: 'Content Calendar — Pixie Lab', robots: { index: false, follow: false } };
export const dynamic = 'force-dynamic';

export default async function MarketingCalendarPage() {
  const guard = await guardPermission('marketing.view');
  if (!guard.ok) return <AccessRestricted what="Marketing" />;
  return <MarketingWorkspace tab="calendar" tenant={tenantForMembership(guard.membership)} />;
}
