...
export const metadata: Metadata = { title: 'Brand Brain — Pixie Lab', robots: { index: false, follow: false } };
export const dynamic = 'force-dynamic';

export default async function MarketingBrandBrainPage() {
  const guard = await guardPermission('marketing.view');
  if (!guard.ok) return <AccessRestricted what="Marketing" />;
  return <MarketingWorkspace tab="brand-brain" tenant={tenantForMembership(guard.membership)} />;
}
