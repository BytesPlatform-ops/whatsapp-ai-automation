...
/** GET tasks (?status=). POST { action:'create'|'complete'|'update', id?, ... }. */
export async function GET(req: Request) {
  const g = await guard('receptionist.view');
  if (!g.ok) return g.response;
  const status = new URL(req.url).searchParams.get('status') || undefined;
  const r = await backendGet(`${A}/tasks`, g.tenant, { status });
...
export async function POST(req: Request) {
  ...
  const tid = String(id || '');
  let r;
  if (action === 'create') r = await backendSend('POST', `${A}/tasks`, g.tenant, rest);
  else if (action === 'complete' && tid) r = await backendSend('POST', `${A}/tasks/${encodeURIComponent(tid)}/complete`, g.tenant, {});
  else if (action === 'update' && tid) r = await backendSend('PATCH', `${A}/tasks/${encodeURIComponent(tid)}`, g.tenant, rest);
