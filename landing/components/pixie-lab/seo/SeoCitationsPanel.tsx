...
import {
  FileText, Download, Upload, RefreshCw, Loader2, ExternalLink,
...
const ACCENT = '#14B8A6';
/** Number of citations shown per page. Keeps DOM bounded for 5000+ rows. */
const PAGE_SIZE = 50;
...
function ConsistencyBadge({ consistent }: { consistent?: boolean | null }) {
  if (consistent === true)
    return (
      ...
      >
        <CheckCircle2 size={9} aria-hidden="true" /> Consistent
      </span>
    );
  if (consistent === false)
    return (
      ...
      >
        <AlertTriangle size={9} aria-hidden="true" /> Inconsistent
      </span>
    );
...
function ClaimedBadge({ claimed }: { claimed?: boolean | null }) {
  ...
  if (claimed === false)
    return <span className="rounded-full px-1.5 py-0.5 text-[10px] font-bold text-[var(--pl-text-muted)]">Unclaimed</span>;
  return null;
}
...
export function SeoCitationsPanel({ initialLocationId }: { initialLocationId?: string }) {
  ...
  const load = useCallback(
    async (off: number) => {
      setStatus('loading');
      setAnnouncement('Loading citations…');
      const env = await seoApi.citations({ location_id: initialLocationId });
      if (!env.backendUp) {
        setStatus('offline');
        setAnnouncement('Citations service offline.');
        return;
      }
      const all = env.citations ?? [];
      // Client-side filtering (the proxy doesn't support consistent/claimed filters
      // as query params in all environments, so we filter locally after fetching).
      // For very large sets the proxy should ideally paginate server-side; we
      // page the filtered result client-side to keep the DOM bounded.
      ...
      setOffset(Math.min(off, Math.max(0, filtered.length - 1)));
      setStatus('done');
      setAnnouncement(`${filtered.length} citation${filtered.length !== 1 ? 's' : ''} loaded.`);
    },
    [initialLocationId, consistentFilter, claimedFilter],
  );
  ...
  const pageSlice = citations.slice(offset, offset + PAGE_SIZE);
      ...
      <div className="mt-6">
        <OfflineState
          ...
          action={
            <button
              onClick={() => load(0)}
              className="rounded-lg border border-[var(--pl-border)] px-4 py-2 text-[13px] font-semibold text-[var(--pl-text-soft)]"
            >
              Retry
            </button>
          }
  ...
  return (
    <div className="mt-6 space-y-5">
      ...
      {total > 0 && (
        <div
          className="flex flex-wrap items-center gap-4 rounded-xl border border-[var(--pl-border)] bg-[var(--pl-surface)] px-4 py-3"
          role="region"
          aria-label="Citations summary"
        >
          <FileText size={14} style={{ color: ACCENT }} aria-hidden="true" />
          <span className="text-[13px] text-[var(--pl-text-soft)]">{total} citation{total !== 1 ? 's' : ''}</span>
          {inconsistentCount > 0 && (
      ...
      {/* Action bar */}
      <div className="flex flex-wrap items-center gap-2" role="toolbar" aria-label="Citations filters and actions">
        {/* Filters */}
        <label className="sr-only" htmlFor="citations-consistent-filter">Consistency filter</label>
        <select
          id="citations-consistent-filter"
        ...
        <label className="sr-only" htmlFor="citations-claimed-filter">Claimed filter</label>
        <select
          id="citations-claimed-filter"
          value={claimedFilter}
        ...
        <div className="ml-auto flex items-center gap-2">
          {initialLocationId && (
            <button
              onClick={handleCheck}
              disabled={checking}
              aria-label={checking ? 'Checking citation consistency…' : 'Check citation consistency'}
            ...
            >
              <RefreshCw size={12} className={checking ? 'animate-spin' : ''} style={{ color: ACCENT }} aria-hidden="true" />
          ...
          {initialLocationId && (
            <label
              ...
              aria-label={importing ? 'Importing citations…' : 'Import citations from CSV'}
            >
              {importing
        ...
        <>
          <div className="overflow-hidden rounded-xl border border-[var(--pl-border)]">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px] text-[12.5px]" aria-label="Citations table">
                <thead>
                  <tr className="border-b border-[var(--pl-border)] bg-[var(--pl-surface-soft)]">
                    {['Directory', 'Name Listed', 'Address Listed', 'Phone Listed', 'Consistent', 'Claimed', 'Last Checked'].map((h) => (
                      <th key={h} scope="col" className="px-3 py-2 text-left font-semibold text-[var(--pl-text-muted)]">
                        {h}
                      </th>
                    ))}
                  </tr>
                        ...
                        {c.listing_url
                          ? (
                            <a
                              ...
                              className="inline-flex items-center gap-1 font-medium text-[var(--pl-text-soft)] underline"
                              aria-label={`${c.directory ?? 'Directory'} (opens in new tab)`}
                            >
                              {c.directory ?? '—'}
                              <ExternalLink size={10} style={{ color: ACCENT }} aria-hidden="true" />
                            </a>
                          )
          ...
          {/* Pagination controls */}
          {totalPages > 1 && (
            <nav aria-label="Citations pagination" className="flex items-center justify-between gap-3 pt-1">
              <span className="text-[12px] text-[var(--pl-text-muted)]">
                Showing {offset + 1}–{Math.min(offset + PAGE_SIZE, total)} of {total}
              </span>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => { const next = offset - PAGE_SIZE; setOffset(next); load(next); }}
                  disabled={offset === 0}
