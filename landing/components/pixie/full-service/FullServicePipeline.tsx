...
export function FullServicePipeline({ unit }: { unit: PipelineUnit }) {
  ...
  return (
    <div className="min-h-screen bg-[#02070a] text-white" style={{ ['--accent' as string]: accent }}>
      <ScrollSectionNavigator accent={accent} />
      <div className="pointer-events-none fixed inset-x-0 top-0 h-[460px]" style={{ background: `radial-gradient(80% 100% at 50% 0%, color-mix(in srgb, ${accent} 16%, transparent), transparent 62%)` }} />

      {/* header + hero */}
      <header className="relative z-10 mx-auto flex max-w-[min(1720px,94vw)] items-center justify-between px-5 py-5 sm:px-8">
        <Link href={unit.dashboardPath} className="inline-flex items-center gap-1.5 rounded-full border border-white/12 bg-white/[0.04] px-3.5 py-2 text-xs font-semibold text-white/75 transition hover:text-white">
      ...
      <main className="relative z-10 mx-auto max-w-[min(1720px,94vw)] px-5 pb-24 sm:px-8">
        ...
        <p className="mt-2 max-w-2xl text-[15px] text-white/55">{pipeline?.subtitle ?? 'Pixie is preparing the work and waiting for your approval before anything goes live.'}</p>
              ...
              {data.approvals.map((a, i) => (
                <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-white/40">{a.channels}</span>
                  </div>
                  <p className="mt-2 font-display text-[15px] font-bold">{a.title}</p>
                  <p className="mt-1 text-[13px] text-white/55">{a.willDo}</p>
                  <div className="mt-3 flex gap-2">
                    ...
                    <button onClick={() => flash('Opened editor')} className="rounded-full border border-white/12 bg-white/[0.04] px-3.5 py-1.5 text-[12px] font-semibold text-white/75">Edit</button>
                    <button onClick={() => flash('Skipped')} className="rounded-full px-3 py-1.5 text-[12px] font-semibold text-white/45">Skip</button>
                  </div>
                </div>
              ))}
            </div>
              ...
              {data.workflows.map((w, i) => (
                <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-full border border-[#25D366]/30 bg-[#25D366]/10 px-2 py-0.5 text-[10px] font-bold text-[#7ef0a8]">{w.status}</span>
                  </div>
                  <p className="mt-1.5 text-[12.5px] text-white/55"><b className="text-white/70">Detects:</b> {w.detects}</p>
                  <p className="text-[12.5px] text-white/55"><b className="text-white/70">Prepares:</b> {w.prepares}</p>
        ...
        {data && (
          <div className="mt-12 grid gap-6 lg:grid-cols-2">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {data.intelligence.map((it, i) => (
                  <div key={i} className="rounded-xl border border-white/10 bg-white/[0.03] p-3.5">
                    <p className="text-[11px] uppercase tracking-wider text-white/40">{it.title}</p>
                    <p className="mt-1 text-[14px] font-semibold text-white">{it.value}</p>
                  </div>
                ))}
              </div>
            </div>
            <div id="activity" className="scroll-mt-24">
              ...
              <ol className="mt-4 space-y-2.5 border-l border-white/10 pl-5">
                {data.activity.map((a, i) => (
                  <li key={i} className="relative text-[13.5px] text-white/70">
          ...
          <>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {data.nextActions.map((n, i) => (
                <div key={i} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                  <div>
                    <p className="font-display text-[14.5px] font-bold">{n.title}</p>
                    <p className="mt-1 text-[12px] text-white/45">Impact {n.impact} · Effort {n.effort} · {n.permission}</p>
                  </div>
      ...
      {toast && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full border border-white/12 bg-[#0a0e16] px-4 py-2.5 text-[13px] text-white shadow-2xl">{toast}</motion.div>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 backdrop-blur-md">
            ...
            return (
              <div key={st.id} className="flex items-center">
                <button onClick={() => setSelected(i)} className="flex w-[132px] flex-col items-center text-center">
                  <span className="grid h-11 w-11 place-items-center rounded-full border-2" style={{ borderColor: done || current || sel ? accent : 'rgba(255,255,255,0.15)', background: current ? accent : done ? `${accent}26` : 'rgba(255,255,255,0.03)', boxShadow: current ? `0 0 22px ${accent}66` : 'none', color: current ? '#02070a' : done ? accent : 'rgba(255,255,255,0.6)' }}>
                    {done ? <Check size={17} /> : current ? <Plane size={17} className="-rotate-45" /> : <span className="text-[12px] font-bold">{i + 1}</span>}
                  </span>
                  <span className={`mt-2 text-[11.5px] font-semibold leading-tight ${sel || current ? 'text-white' : 'text-white/55'}`}>{st.title}</span>
                </button>
                {i < stages.length - 1 && <div className="mx-1 h-0.5 w-10 sm:w-14" style={{ background: done ? accent : 'rgba(255,255,255,0.12)' }} />}
              </div>
            );
          })}
        </div>
      ...
      <div className="mt-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
        ...
        <p className="mt-1 text-[13.5px] text-white/60">{stage.description}</p>
        <p className="mt-2 text-[12px] text-white/40">Your role: approve / edit / skip · Pixie’s role: detect / prepare / execute / learn</p>
      </div>
    </div>
  );
}
