...
Tenant is caller-supplied (Query for reads, body field for writes) exactly like
the rest of the backend — the Next.js proxy injects it server-side and the
internal-secret middleware guards the whole app.
"""
...
def run_message(body: MessageBody) -> dict:
...
def list_conversations(tenant_id: str = Query("demo_tenant")) -> dict:
    return {"conversations": stores.conversations().list(tenant_id)}


@console_router.get("/conversations/{conversation_id}")
def get_conversation(conversation_id: str, tenant_id: str = Query("demo_tenant")) -> dict:
...
def add_conversation_message(conversation_id: str, body: MessageBody) -> dict:
...
def escalate_conversation(conversation_id: str, body: EscalateBody) -> dict:
    conv = _require(stores.conversations(), tenant_id=body.tenant_id,
                    record_id=conversation_id, label="conversation")
    esc = Escalation(
        tenant_id=body.tenant_id, contact_id=conv.get("contact_id"),
        conversation_id=conversation_id, reason=body.reason or "Manual escalation",
        source=conv.get("channel", "web_chat"),
    ).model_dump()
    notified: list[str] = []
    try:
        note = providers.notify_team(tenant_id=body.tenant_id,
    ...
    stores.escalations().put(body.tenant_id, esc)
    conv["status"] = "escalated"
    stores.conversations().put(body.tenant_id, conv)
...
def list_leads(tenant_id: str = Query("demo_tenant"),
               status: Optional[str] = Query(None)) -> dict:
...
def get_lead(lead_id: str, tenant_id: str = Query("demo_tenant")) -> dict:
...
def update_lead(lead_id: str, body: LeadPatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    return {"lead": _patch(stores.contacts(), body.tenant_id, lead_id, patch, "lead")}
...
def lead_follow_up(lead_id: str, body: FollowUpBody) -> dict:
    lead = _require(stores.contacts(), body.tenant_id, lead_id, "lead")
    task = Task(
        tenant_id=body.tenant_id, contact_id=lead_id, kind="follow_up",
    ...
    stores.tasks().put(body.tenant_id, task)
    lead["status"] = "follow_up_needed"
    stores.contacts().put(body.tenant_id, lead)
    return {"task": task}
...
def list_bookings(tenant_id: str = Query("demo_tenant")) -> dict:
...
def create_booking(body: BookingCreate) -> dict:
    booking = Booking(**body.model_dump(), source="manual").model_dump()
    stores.bookings().put(body.tenant_id, booking)
...
def update_booking(booking_id: str, body: BookingPatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    return {"booking": _patch(stores.bookings(), body.tenant_id, booking_id, patch, "booking")}
...
def confirm_booking(booking_id: str, body: TenantBody) -> dict:
    booking = _require(stores.bookings(), body.tenant_id, booking_id, "booking")
...
def cancel_booking(booking_id: str, body: TenantBody) -> dict:
    booking = _require(stores.bookings(), body.tenant_id, booking_id, "booking")
...
def list_quotes(tenant_id: str = Query("demo_tenant")) -> dict:
...
def create_quote(body: QuoteCreate) -> dict:
    quote = Quote(**body.model_dump(), source="manual").model_dump()
    stores.quotes().put(body.tenant_id, quote)
    return {"quote": quote}
...
def update_quote(quote_id: str, body: QuotePatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    return {"quote": _patch(stores.quotes(), body.tenant_id, quote_id, patch, "quote")}


# ── tasks + reminders ─────────────────────────────────────────────────────────

@console_router.get("/tasks")
def list_tasks(tenant_id: str = Query("demo_tenant"),
               status: Optional[str] = Query(None)) -> dict:
...
def create_task(body: TaskCreateBody) -> dict:
    task = Task(**body.model_dump(), source="manual").model_dump()
    stores.tasks().put(body.tenant_id, task)
    return {"task": task}
...
def update_task(task_id: str, body: TaskPatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    return {"task": _patch(stores.tasks(), body.tenant_id, task_id, patch, "task")}


@console_router.post("/tasks/{task_id}/complete")
def complete_task(task_id: str, body: TenantBody) -> dict:
    task = _require(stores.tasks(), body.tenant_id, task_id, "task")
    task["status"] = "done"
    stores.tasks().put(body.tenant_id, task)
    return {"task": task}
...
def list_reminders(tenant_id: str = Query("demo_tenant")) -> dict:
...
def list_tickets(tenant_id: str = Query("demo_tenant")) -> dict:
...
def create_ticket(payload: TicketCreate) -> dict:
    ticket = Ticket(
        tenant_id=payload.tenant_id, kind=payload.kind, subject=payload.subject,
        body=payload.body, priority=payload.priority, contact_id=payload.contact_id,
        source="manual",
    ).model_dump()
    stores.tickets().put(payload.tenant_id, ticket)
...
def update_ticket(ticket_id: str, body: TicketPatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    return {"ticket": _patch(stores.tickets(), body.tenant_id, ticket_id, patch, "ticket")}


@console_router.get("/escalations")
def list_escalations(tenant_id: str = Query("demo_tenant")) -> dict:
...
def update_escalation(escalation_id: str, body: EscalationPatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    return {"escalation": _patch(stores.escalations(), body.tenant_id, escalation_id, patch, "escalation")}


# ── payments ──────────────────────────────────────────────────────────────────

@console_router.get("/payments")
def list_payments(tenant_id: str = Query("demo_tenant")) -> dict:
...
def create_payment_link(body: PaymentLinkBody) -> dict:
    payment = PaymentRequest(
        tenant_id=body.tenant_id, contact_id=body.contact_id, amount=body.amount,
    ...
    stores.payments().put(body.tenant_id, payment)
    try:
...
async def payment_webhook(request: Request) -> dict:
    """Best-effort Stripe webhook: mark the matching payment paid on
    checkout.session.completed. (Signature verification + a public route are a
    production follow-up; documented in the report.)"""
    try:
...
def list_campaigns(tenant_id: str = Query("demo_tenant")) -> dict:
    try:
...
def campaign_replies(campaign_id: str, tenant_id: str = Query("demo_tenant")) -> dict:
...
def ingest_campaign_reply(body: CampaignReplyIngest) -> dict:
    ...
    return engine.run_message(
        tenant_id=body.tenant_id, message=body.message, channel="campaign",
...
def send_campaign(campaign_id: str, body: CampaignSend) -> dict:
    ...
    try:
        ...
        campaign = cstore.get_campaign(body.tenant_id, campaign_id)
        if campaign is None:
...
def get_business_profile(tenant_id: str = Query("demo_tenant")) -> dict:
...
def update_business_profile(body: ProfilePatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    return {"profile": bp.save_profile(body.tenant_id, patch)}


@console_router.get("/knowledge")
def list_knowledge(tenant_id: str = Query("demo_tenant")) -> dict:
...
def create_knowledge(body: KnowledgeCreate) -> dict:
    return {"item": bp.create_knowledge(body.tenant_id, title=body.title, content=body.content,
...
def update_knowledge(item_id: str, body: KnowledgePatch) -> dict:
    patch = body.model_dump(exclude={"tenant_id"}, exclude_none=True)
    item = bp.update_knowledge(body.tenant_id, item_id, patch)
    if item is None:
...
def delete_knowledge(item_id: str, tenant_id: str = Query("demo_tenant")) -> dict:
...
def overview(tenant_id: str = Query("demo_tenant")) -> dict:
    return analytics.overview(tenant_id)


@console_router.get("/integrations/status")
def integrations_status(tenant_id: str = Query("demo_tenant")) -> dict:
    r = get_router()
...
def integrations_test(body: IntegrationTest) -> dict:
    cap = body.capability
    if cap == "webhooks":
        return {"capability": cap, "result": providers.emit_webhook(
            event="test.ping", payload={"tenant_id": body.tenant_id, "ping": True}).to_dict()}
    ...
    core = {c["capability"]: c for c in integration_status(body.tenant_id, AGENT_SLUG)}
    if cap in core:
